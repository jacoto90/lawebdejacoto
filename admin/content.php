...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!app_verify_csrf_token((string) ($_POST['csrf_token'] ?? ''))) {
        ...
        if ($action === 'save_skill' || $action === 'add_skill') {
            ...
            app_upsert($pdo, 'portfolio_skills', ['id'], ['id' => $id, 'name' => admin_post('name'), 'sort_order' => admin_int('sort_order'), 'is_active' => admin_bool('is_active')]);
        ...
        if ($action === 'save_experience' || $action === 'add_experience') {
            ...
            app_upsert($pdo, 'portfolio_experiences', ['id'], ['id' => $id, 'company' => admin_post('company'), 'sort_order' => admin_int('sort_order'), 'is_active' => admin_bool('is_active')]);
            $periodEs = admin_lang_value('period', 'es');
            $titleEs = admin_lang_value('title', 'es');
...
<main class="admin-editor-shell">
    ...
    <?php if ($section === 'skills'): ?>
        <section class="admin-editor-card"><h2>Skills</h2><?php foreach (admin_rows($pdo, 'SELECT * FROM portfolio_skills ORDER BY sort_order ASC, name ASC') as $row): ?>
            <form class="admin-inline-form" method="post"><input type="hidden" name="csrf_token" value="<?= e($csrf) ?>"><input type="hidden" name="id" value="<?= e($row['id']) ?>"><input type="hidden" name="action" value="save_skill"><input name="name" value="<?= e($row['name']) ?>"><input name="sort_order" type="number" value="<?= e((string) $row['sort_order']) ?>"><label class="admin-check"><input type="checkbox" name="is_active" <?= (int) $row['is_active'] === 1 ? 'checked' : '' ?>> activo</label><button class="btn btn-primary">Guardar</button><button class="btn btn-ghost" name="action" value="delete_skill" onclick="return confirm('Eliminar skill?')">Eliminar</button></form>
        <?php endforeach; ?><form class="admin-inline-form admin-add-form" method="post"><input type="hidden" name="csrf_token" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="add_skill"><input name="name" placeholder="Nueva skill"><input name="sort_order" type="number" value="99"><label class="admin-check"><input type="checkbox" name="is_active" checked> activo</label><button class="btn btn-primary">Añadir</button></form></section>
    <?php endif; ?>
    ...
    <?php if ($section === 'experience'): ?>
        ...
        <?php endforeach; ?><form class="admin-edit-row admin-form" method="post"><input type="hidden" name="csrf_token" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="add_experience"><h3>Nueva experiencia</h3><div class="admin-grid-two"><label>Empresa<input name="company"></label><label>Orden<input name="sort_order" type="number" value="99"></label><label>Tags<input name="tags"></label><label class="admin-check"><input type="checkbox" name="is_active" checked> activo</label><label>Periodo ES<input name="period_es"></label><label>Periodo EN<input name="period_en"></label><label>Título ES<input name="title_es"></label><label>Título EN<input name="title_en"></label><label>Resumen ES<textarea name="summary_es" rows="3"></textarea></label><label>Resumen EN<textarea name="summary_en" rows="3"></textarea></label></div><button class="btn btn-primary">Añadir experiencia</button></form></section>
    <?php endif; ?>
</main>
</body>
</html>
